...
export default function Board() {
  ...
  useEffect(() => {
    try { const s = localStorage.getItem("bitmain-na-sales-done"); if (s) setDone(JSON.parse(s)); } catch {}
    setHydrated(true);
  }, []);
  useEffect(() => {
    if (!hydrated) return;
    try { localStorage.setItem("bitmain-na-sales-done", JSON.stringify(done)); } catch {}
  }, [done, hydrated]);
    ...
    <div style={{ minHeight: "100vh", background: "#F5F5F0", color: "#1a1a1a", fontFamily: "'Noto Sans SC', -apple-system, sans-serif" }}>
      <header style={{ position: "sticky", top: 0, zIndex: 40, background: "rgba(255,255,255,0.95)", backdropFilter: "blur(16px)", borderBottom: "1px solid #e0e0e0", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "14px 20px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            <div>
              <h1 style={{ margin: 0, fontSize: 20, fontWeight: 900, letterSpacing: -0.5, color: "#B45309" }}>
                BITMAIN 北美机构销售 · 面试答题板
              </h1>
              <p style={{ margin: "3px 0 0", fontSize: 11, color: "#888", letterSpacing: 0.5 }}>
                {total}题 · {modules.length}模块 · 市场简报 · STAR行为 · 销售策略 · 反问环节
              </p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 120, height: 6, borderRadius: 3, background: "#e8e8e4", overflow: "hidden" }}>
                <div style={{ height: "100%", borderRadius: 3, background: pct === 100 ? "#2E7D32" : "#B45309", width: pct + "%", transition: "width 0.5s ease" }} />
              </div>
              <span style={{ fontSize: 13, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: pct === 100 ? "#2E7D32" : "#B45309" }}>
                {doneN}/{total}
              </span>
              <button onClick={() => setShowStats(!showStats)} style={{ padding: "3px 8px", borderRadius: 6, border: "1px solid #ddd", background: showStats ? "#FEF3C7" : "#fff", color: showStats ? "#B45309" : "#888", fontSize: 11, cursor: "pointer" }}>📊</button>
            </div>
          </div>
        ...
        {visMods.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <button onClick={() => { setQ(""); setModF("all"); setTypeF("all"); setUndone(false); }}
              style={{ marginTop: 8, fontSize: 12, color: "#B45309", background: "none", border: "none", textDecoration: "underline", cursor: "pointer" }}>清除筛选</button>
          </div>
        )}
      </main>
      ...
      <footer style={{ textAlign: "center", padding: "20px", borderTop: "1px solid #e8e8e4" }}>
        <p style={{ fontSize: 10, color: "#bbb", lineHeight: 1.6 }}>
          {total}题 · {modules.length}模块 · 行为·销售·技术·反问·商业战略全覆盖<br/>核心原则: 诚实应对差距 · 数据驱动 · S23 / 美国工厂 · 复利式客户关系
        </p>
      </footer>
